...
authRouter.post(
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      return res
        .status(404)
        .json({ message: "incomplete details, all fields required!" });
      ...
      try {
        ...
        const result = await User.create(req.body); //creating the user in the database
        ...
        return res
          .status(400)
          .json({ message: "error occurred in registration", error: error });
      }
    }
  }
);
...
authRouter.post(
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) { //checking if there are any errors

      console.log(errors.array());  
      return res.status(400).json({ errors: errors.array() });  //return the response with the error
      ...
      try {
        ...
        if(!user){
            return res.status(400).json({message:"Authentication failed, invalid email!"})
        }
        ...
        if (user.password === req.body.password) {
          ...
          return res
            .status(200)
            .json({ message: "User successfully logged in", result: user });

        } else {  //if the passwod does not matches, the we return the response with an error
          return res
            .status(400)
            .json({ message: "User authentication failed, Invalid Password!" });
        }
      } catch (error) {
        console.log("Error in login", error);
        return res
          .status(400)
          .json({ message: "Error occured in login", result: error });
      }
    }
  }
);
